'use client'

import { Folder } from 'lucide-react'
import { memo } from 'react'

interface CategoryNoResultsProps {
	onClearSearch: () => void
}

const CategoryNoResultsComponent = ({ onClearSearch }: CategoryNoResultsProps) => {
	return (
		<div className="flex items-center justify-center min-h-[calc(100vh-24rem)] py-12">
			<div className="w-full max-w-md text-center space-y-6 px-4">
				<div className="flex justify-center">
					<div className="relative">
						<div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-primary/10 to-primary/5 border-2 border-primary/20 flex items-center justify-center">
							<Folder className="w-12 h-12 text-primary/50" />
						</div>
						<div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-primary/20 animate-pulse" />
					</div>
				</div>
				<div className="space-y-2">
					<h3 className="text-2xl font-bold text-foreground">
						No Categories Found
					</h3>
					<p className="text-sm text-muted-foreground max-w-xs mx-auto">
						No categories match your search. Try a different keyword.
					</p>
				</div>
				<button
					type="button"
					onClick={onClearSearch}
					className="px-5 py-2.5 bg-background hover:bg-accent border-2 border-border hover:border-primary text-foreground rounded-xl font-medium transition-all"
				>
					Clear Search
				</button>
			</div>
		</div>
	)
}

export const CategoryNoResults = memo(CategoryNoResultsComponent)
